...
import {
  BarChart3, DollarSign, ExternalLink, Eye, Radio, RefreshCw,
...
function LiveYouTubeCard({ live, liveErr, forcing, onForce, onReconnect }) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button data-testid="yt-live-refresh-button" size="sm" variant="outline" disabled={forcing}
            onClick={onForce} className="border-white/10 bg-black/30 text-xs text-slate-300 hover:bg-white/5">
        ...
        <>
          ...
          {a.scope_ok === false && (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-500/20 bg-blue-950/10 px-4 py-3 text-xs text-slate-400">
              ...
              <Button size="sm" variant="outline" onClick={onReconnect} className="border-amber-500/40 bg-amber-500/10 text-xs text-amber-300 hover:bg-amber-500/20">
                Reconnect YouTube
              </Button>
            </div>
          )}
...
function PerformanceTable({ videos }) {
  return (
      ...
      <div data-testid="admin-accounts-table" className="max-h-[360px] overflow-y-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            {!(accounts || []).length && <tr><td colSpan={4} className="px-6 py-10 text-center text-sm text-slate-500">No accounts yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
      ...
      <div data-testid="admin-channels-table" className="max-h-[360px] overflow-y-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            {!(channels || []).length && <tr><td colSpan={4} className="px-6 py-10 text-center text-sm text-slate-500">No channels</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
export default function AdminPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <LiveYouTubeCard live={live} liveErr={liveErr} forcing={forcing} onForce={forceLive} onReconnect={reconnectYouTube} />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat testid="admin-stat-trend" icon={TrendingUp} label="Views (28d, live)" value={num(live?.analytics?.views_28d)} />
      </div>
