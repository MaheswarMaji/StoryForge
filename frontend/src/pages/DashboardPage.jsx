...
const TOOLTIP_STYLE = { background: "#1A1D2E", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 12, color: "#F8FAFC" };
...
function CostPieCard({ cost, costPie }) {
  return (
    <div className="card-glow rounded-2xl border border-amber-500/10 bg-[#12141F] p-6 lg:col-span-2">
      ...
      <p className="mb-4 text-xs text-slate-500">Total spend ${(cost?.total || 0).toFixed(3)}</p>
      {costPie.length ? (
        <ResponsiveContainer width="100%" height={260}>
          <PieChart>
            <Pie data={costPie} dataKey="value" nameKey="name" innerRadius={55} outerRadius={90} paddingAngle={4} stroke="none">
              {costPie.map((_, i) => <Cell key={`slice-${costPie[i].name}`} fill={COLORS[i % COLORS.length]} />)}
            </Pie>
...
function BeatCoverageCard({ beatData }) {
  return (
    <div className="card-glow rounded-2xl border border-amber-500/10 bg-[#12141F] p-6 lg:col-span-2">
...
function JobQueuePanel({ dash, refresh }) {
  const queue = dash.queue || {};
  const togglePause = async () => {
    try {
      await api.post("/queue/pause", { paused: !queue.paused });
      toast.success(queue.paused ? "Queue resumed" : "Queue paused — running jobs continue, no new jobs start");
      refresh();
    } catch { toast.error("Failed"); }
  };
  const cancel = async (id) => {
    try { await api.post(`/jobs/${id}/cancel`); toast.success("Job stopped"); refresh(); }
    ...
    <div className="card-glow overflow-hidden rounded-2xl border border-amber-500/10 bg-[#12141F] lg:col-span-3">
      <div className="flex items-center justify-between p-6 pb-3">
        ...
        <div className="flex items-center gap-2">
          {queue.paused && <Badge className="border border-orange-600/50 bg-orange-950/50 text-[10px] text-orange-300">paused</Badge>}
          <button data-testid="queue-pause-button" onClick={togglePause}
            className={`rounded-full border px-3 py-1 text-[10px] font-semibold ${queue.paused ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300" : "border-orange-500/40 bg-orange-500/10 text-orange-300"}`}>
              ...
              <tr key={j.id} data-testid="job-row" className="border-b border-white/5 text-slate-300">
                ...
                <td className="py-2 pr-1 text-right">
                  {["queued", "running"].includes(j.status) && (
                    <button data-testid={`job-cancel-${j.id}`} title="Stop this job" onClick={() => cancel(j.id)}
                      className="rounded-md border border-rose-500/30 px-2 py-0.5 text-[10px] font-semibold text-rose-300 hover:bg-rose-500/10">Stop</button>
                  )}
                </td>
              </tr>
            ))}
...
function RecentStories({ stories, chName }) {
  return (
    <div>
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <CostPieCard cost={dash.cost} costPie={costPie} />
      </div>

      <div className="grid gap-6 lg:grid-cols-5">
        <BeatCoverageCard beatData={beatData} />
        <JobQueuePanel dash={dash} refresh={refresh} />
      </div>

      <RecentStories stories={dash.recent_stories} chName={chName} />
    </div>
  );
}
