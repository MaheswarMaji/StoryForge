...
- 2026-09-13 (latest): Pexels key wired (free real photos composite into storyboard panels; HD portrait stock clips in video chain) + **Stability AI wired and FUNDED (working)**: image chain now openai → stability → hf → fal → replicate → emergent → gemini → qwen_local → pexels → procedural; storyboard poster renders REAL AI art (1 Stability call per video, verified '[storyboard] AI poster generated'); vault + Settings labels added; hex magic-byte bug fixed. (iteration_7: 10/10 after vault fix)
